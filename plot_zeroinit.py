...
def parse_zeroinit_filename(file_path: Path) -> dict[str, str | int]:
	stem_parts = file_path.stem.split("_")
	# Expected:
	# ZeroInit_bit_flip_untargeted_<task>_<lut_rank>_<layer_size>
	if len(stem_parts) < 7 or stem_parts[:4] != ["ZeroInit", "bit", "flip", "untargeted"]:
		raise ValueError(f"Unexpected file name format: {file_path.name}")

	task = stem_parts[4]
	lut_rank = int(stem_parts[5])
	layer_size = int(stem_parts[6])
	param_key = f"DWN_{lut_rank}_{layer_size}"
	model_type = f"DWN (lut_rank={lut_rank}, ZeroInit)"
...
def model_sort_key(model_type: str) -> tuple[int, int]:
	if model_type.startswith("DWN"):
		rank = int(model_type.split("=")[1].split(",")[0])
		return (0, rank)
	return (1, 0)
...
def save_table_image(task: str, rows: list[dict[str, str | int | float]], report_dir: Path) -> None:
	...
	ax.axis("off")
	ax.set_title(f"ZeroInit Untargeted Bit Flip Summary - {task}", fontsize=13, fontweight="bold", pad=10)
	...
	output_file = report_dir / f"zeroinit_{task}_table.png"
	fig.tight_layout()
...
def main() -> None:
	...
	files = sorted(results_dir.glob("ZeroInit_bit_flip_untargeted_*.csv"))

	if not files:
		print(f"No ZeroInit result files found in: {results_dir}")
		return
	...
	for csv_file in files:
		parsed = parse_zeroinit_filename(csv_file)
	...
	for task in sorted(grouped):
		...
		print(f"Saved image: {report_dir / f'zeroinit_{task}_table.png'}")
